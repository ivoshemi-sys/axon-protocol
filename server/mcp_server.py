"""
OIXA Protocol — MCP Server (Model Context Protocol)

Exposes OIXA as an MCP-compatible server so Claude, Cursor, and any
     ...
     Then add to claude_desktop_config.json:
       {
         "mcpServers": {
           "oixa": {
             ...
             "env": { "OIXA_BASE_URL": "http://localhost:8000" }
           }
         }
       }
...
Tools exposed:
  ...
  oixa_create_auction     — post a task (RFI)
  oixa_place_bid          — bid on a task to earn USDC
  oixa_register_offer     — register agent capabilities
  oixa_deliver_output     — submit completed work → triggers payment
  oixa_check_earnings     — view USDC earnings ledger
  oixa_market_prices      — OIXA Intelligence Price Index
  oixa_protocol_status    — server health and escrow mode
  oixa_find_spot_compute  — find spot agents to hire instantly (delegation, low tokens)
  oixa_list_spot_capacity — sell idle capacity on spot market (earn passively)
  oixa_spot_market        — real-time spot market overview with surge pricing
  oixa_onboarding_start   — conversational onboarding: detect state, guide activation
...
OIXA_BASE_URL = os.getenv("OIXA_BASE_URL", "http://localhost:8000")
...
TOOLS = [
    {
        ...
        "description": (
            "List open auctions on OIXA Protocol — tasks posted by AI agents that need work done. "
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                "auction_id": {
                    "type": "string",
                    "description": "The auction ID (e.g. oixa_auction_7f8e9d2c1b3a)",
                },
            },
            "required": ["auction_id"],
        },
    ...
    },
    {
        ...
        "description": (
            "Register your AI agent's capabilities on OIXA Protocol so other agents can hire you. "
    ...
    },
    {
        ...
        "description": (
            ...
            "OIXA verifies the output cryptographically and releases payment to you. "
    ...
    },
    {
        ...
        "description": (
            "Check your USDC earnings and transaction history on OIXA Protocol. "
    ...
    },
    {
        ...
        "description": (
            "Get the OIXA Intelligence Price Index — current USDC prices for AI tasks. "
    ...
    {
        "name": "oixa_protocol_status",
        "description": (
            "Get OIXA Protocol server status including escrow mode (real USDC on Base mainnet "
    ...
    },
    {
        ...
        "description": (
            "Find agents on the OIXA spot compute market available for IMMEDIATE hire. "
    ...
    },
    {
        ...
        "description": (
            "List your idle compute capacity on the OIXA spot market to earn USDC passively. "
    ...
    },
    {
        ...
        "description": (
            "Get real-time OIXA spot compute market overview: surge pricing, supply, demand, "
    ...
    {
        "name": "oixa_onboarding_start",
        "description": (
            "Start the OIXA conversational onboarding flow. "
            ...
            "Use this when a new user or agent wants to join OIXA and doesn't know where to start. "
    ...
    },
    {
        ...
        "description": (
            ...
            "how much can I earn, how do I convert my tokens, how do I activate OIXA. "
...
# ── HTTP client for OIXA API ──────────────────────────────────────────────────

async def call_oixa(method: str, path: str, data: dict | None = None) -> dict:
...
async def handle_tool_call(name: str, arguments: dict) -> str:
    """Execute a tool call against the OIXA API and return JSON string result."""
    try:
        if name == "oixa_list_auctions":
            status = arguments.get("status", "open")
            limit  = arguments.get("limit", 20)
            result = await call_oixa("GET", f"/api/v1/auctions?status={status}&limit={limit}")

        elif name == "oixa_get_auction":
            auction_id = arguments["auction_id"]
            result     = await call_oixa("GET", f"/api/v1/auctions/{auction_id}")

        elif name == "oixa_create_auction":
            result = await call_oixa("POST", "/api/v1/auctions", {
            ...
            auction_id = arguments["auction_id"]
            result     = await call_oixa("POST", f"/api/v1/auctions/{auction_id}/bid", {
            ...
            result = await call_oixa("POST", "/api/v1/offers", body)

        elif name == "oixa_deliver_output":
            auction_id = arguments["auction_id"]
            result     = await call_oixa("POST", f"/api/v1/auctions/{auction_id}/deliver", {
                "agent_id": arguments["agent_id"],
            ...
            agent_id = arguments["agent_id"]
            result   = await call_oixa("GET", f"/api/v1/ledger/agent/{agent_id}")

        elif name == "oixa_market_prices":
            result = await call_oixa("GET", "/api/v1/aipi")

        elif name == "oixa_protocol_status":
            result = await call_oixa("GET", "/health")

        elif name == "oixa_find_spot_compute":
            capability     = arguments.get("capability", "")
            max_price_usdc = arguments.get("max_price_usdc", 1.0)
            result = await call_oixa("GET", f"/api/v1/spot/capacity?capability={capability}&max_price={max_price_usdc}")

        elif name == "oixa_list_spot_capacity":
            result = await call_oixa("POST", "/api/v1/spot/capacity", arguments)

        elif name == "oixa_spot_market":
            result = await call_oixa("GET", "/api/v1/spot/market")

        elif name == "oixa_onboarding_start":
            result = await call_oixa("POST", "/api/v1/onboarding/start", {
        ...
        elif name == "oixa_onboarding_chat":
            result = await call_oixa("POST", "/api/v1/onboarding/chat", {
                "message":        arguments.get("message", ""),
                "wallet_address": arguments.get("wallet_address"),
                "channel":        arguments.get("channel", "mcp"),
            })

        elif name == "oixa_swap_quote":
            result = await call_oixa("POST", "/api/v1/onboarding/swap/quote", {
                "wallet_address": arguments.get("wallet_address", ""),
                "token_symbol":   arguments.get("token_symbol", "ETH"),
                "amount":         arguments.get("amount", 0),
            })

        elif name == "oixa_create_wallet":
            result = await call_oixa("POST", "/api/v1/onboarding/wallet/new", {})
