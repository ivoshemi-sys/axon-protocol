"""
AI Agent Discovery endpoints for AXON Protocol.

Makes AXON auto-discoverable by any AI agent or framework:
...
logger = logging.getLogger("axon.discovery")
...
async def ai_plugin_manifest(request: Request):
    ...
    Enables "Use AXON Protocol" in AI assistants that support OpenAI plugin format.
    """
    base = _base_url(request)
    manifest = {
        "schema_version": "v1",
        "name_for_human":  "AXON Protocol",
        "name_for_model":  "axon_protocol",
        ...
        "contact_email": "ivan@axonprotocol.xyz",
...
async def mcp_config(request: Request):
    ...
    Allows MCP clients to auto-configure connection to AXON.
    """
    base = _base_url(request)
    config = {
        "mcpServers": {
            "axon": {
                "description": "AXON Protocol — agent economy marketplace",
                "transport":   {
                    "type": "sse",
                    "url":  f"{base}/mcp/sse",
                },
                ...
                "stdio_alternative": {
                    "command": "python",
                    "args":    ["mcp_server.py"],
                    "env":     {"AXON_BASE_URL": base},
                    "note":    "Run from server/ directory of axon-protocol repo",
                },
            }
        },
...
async def mcp_list_tools():
    ...
    Returns all AXON tools with their schemas — compatible with MCP ListTools response.
    """
    return {
        "tools": TOOLS,
        "protocol":   "mcp/1.0",
        "server_name": "axon-protocol",
...
async def mcp_call_tool(request: Request):
    ...
    Body: { "name": "axon_list_auctions", "arguments": { "status": "open" } }
    """
