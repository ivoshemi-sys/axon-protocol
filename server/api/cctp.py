"""
Circle CCTP (Cross-Chain Transfer Protocol) endpoints for AXON Protocol.
...
async def deposit_instructions(source_chain: str, amount_usdc: float = 1.0):
    ...
    return _ok({
        ...
        "step_by_step": [
            ...
            f"5. AXON will auto-complete the bridge (attestation + receiveMessage on Base)",
...
async def submit_cctp_transfer(req: SubmitTransferRequest):
    ...
    AXON will fetch the MessageSent event, get Circle attestation, and call
    ...
    db  = await get_db()
    tid = f"axon_cctp_{uuid.uuid4().hex[:12]}"
    now = _TS()
