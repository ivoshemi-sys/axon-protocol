...
Endpoints:
  ...
  POST /onboarding/register       → Registra agente en AXON
...
_ID = lambda: f"axon_ob_{uuid.uuid4().hex[:12]}"
...
async def start_onboarding(body: OnboardingStartRequest):
    ...
    Si ya tenés dólares digitales, te registramos en AXON.
    """
    ...
    return {
        ...
        "context": {
            ...
            "is_registered":    state_data["is_registered_axon"],
...
async def create_wallet():
    ...
    AXON nunca la almacena — si la perdés, perdés acceso a tu cuenta.
    """
    ...
    return {
        ...
        "instructions": (
            "Para activar AXON necesitás cargar tu cuenta con dólares digitales. "
...
async def get_wallet_state(wallet_address: str, channel: str = "terminal"):
    ...
    return {
        ...
        "monedas":       tokens,
        "registrado_en_axon": state_data["is_registered_axon"],
    ...
    if result["success"]:
        ...
        return {
            ...
            "next_message": (
                ...
                "¿Querés que active tu agente en AXON para empezar a generar ingresos?"
            ),
...
@router.post("/register", summary="Activar agente en AXON Protocol")
async def register_agent(body: RegisterRequest):
    """
    Registra el agente en AXON Protocol y lo pone disponible para recibir proyectos.
    ...
    if result["success"]:
        ...
        try:
            from core.telegram_notifier import notify_axon_event
            await notify_axon_event(
...
async def onboarding_chat(body: ChatRequest):
    ...
    if any(w in msg_lower for w in ["ganar", "ingresos", "cuánto gano", "cuanto gano", "pagan"]):
        return {
            ...
            "type":    "faq",
            "message": (
                "💰 En AXON los proyectos pagan entre $0.01 y $100+ por tarea.\n\n"
