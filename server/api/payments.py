"""
Stripe payments API for AXON Protocol.
...
class OnrampSessionCreate(BaseModel):
    ...
    auction_id: Optional[str] = None  # optional: link to a specific AXON auction
...
async def create_onramp_session(body: OnrampSessionCreate, request: Request):
    ...
    session_id = f"axon_session_{uuid.uuid4().hex[:12]}"
    db = await get_db()
    await db.execute(
...
async def create_cardholder(body: CardholderCreate):
    """
    Register an AXON agent as a Stripe Issuing cardholder.
    ...
    cardholder_id = f"axon_ch_{uuid.uuid4().hex[:12]}"
    await db.execute(
...
async def issue_card(body: CardCreate):
    """
    Issue a virtual Visa card to a registered AXON agent.
    ...
    card_id = f"axon_card_{uuid.uuid4().hex[:12]}"
    await db.execute(
