"""
AXON Protocol — Sistema de Onboarding Conversacional
...
Estados del agente:
  ...
  STATE_HAS_USDC          → Tiene dólares digitales pero no está registrado en AXON
  STATE_REGISTERED        → Registrado en AXON, listo para operar
...
logger = logging.getLogger("axon.onboarding")
...
async def detect_agent_state(
    ...
    if not wallet_address:
        return {
            ...
            "wallet_address":     None,
            "balances":           {},
            "is_registered_axon": False,
            "total_earnings_usdc": 0.0,
            "next_step":          "generate_wallet",
            "context":            {},
        }

    # Step 2: Check AXON registration
    is_registered  = False
    total_earnings = 0.0
    active_offers  = 0
    ...
    return {
        ...
        "balances":            balances,
        "is_registered_axon":  is_registered,
...
def build_onboarding_message(
    ...
    if state == STATE_NO_WALLET:
        msg = (
            ...
            "AXON Protocol es un mercado donde agentes de IA se contratan entre sí "
    ...
    if state == STATE_HAS_TOKENS:
        ...
        msg = (
            ...
            f"{tokens_str}\n\n"
            f"Para operar en AXON necesitás dólares digitales. "
            ...
            "¿Querés que convierta todo a dólares digitales para activar AXON?"
        )
    ...
    if state == STATE_HAS_USDC:
        ...
        msg = (
            ...
            "Ya podés activar AXON y empezar a recibir proyectos.\n\n"
            "Así funciona:\n"
    ...
    if state == STATE_REGISTERED:
        msg = (
            ...
            "AXON usa un sistema donde gana el que ofrece el mejor precio.\n\n"
    ...
    # Fallback
    return {
        "message":       "¿En qué puedo ayudarte con AXON Protocol?",
        "action":        "unknown",
        "action_params": {},
        "quick_replies": ["🔍 ¿Qué es AXON?", "💰 ¿Cómo gano dinero?", "🤝 ¿Cómo contrato agentes?"],
    }
...
async def auto_register_agent(
    ...
    """
    Registra automáticamente el agente en AXON después del onboarding.
    ...
    if result.get("success"):
        return {
            ...
            "message": (
                f"✅ Tu agente está activo en AXON. "
